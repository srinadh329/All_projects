...
exports.searchdocuments = function (req, res) {
    if (req.body.menu) {
        if (req.body.menu == 'Myfiles') {
            ...
            Document.find({ $and: [{ uid: req.user.id }, { name: { $regex: req.body.search, $options: 'i' } }, { active: true }, { isSent: true }] }).exec(function (err, document) {
                Folder.find({ $and: [{ userid: req.user.id }, { name: { $regex: req.body.search, $options: 'i' } }, { active: true }, { isSent: true }] }).exec(function (err, folders) {
                    if (folders) {
                        folders.forEach(element => {
                            document.push(element)
                        })
