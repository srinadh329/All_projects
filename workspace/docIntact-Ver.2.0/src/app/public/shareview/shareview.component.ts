          ...
          this.documentService.getpass(this.linkid).subscribe((data: any) => {
            ...
            if ((this.sharedocument.length == 0 || this.sharedocument.revoke == true) && !this.revokeDialog) {
              ...
              let dialogRef = this.dialog.open(CommonDialogComponent,
                { data: { title: 'dependency', name: 'dependency', content: "You don't Have an Access to View this file" }, width: '500px', panelClass: 'deletemod', disableClose: false });
    ...
    this.dataservice.newMessageReceived()
      .subscribe(data => {
        ...
        if (data && data._id == this.sharedRecord._id && (data.active == false || data.revoke == true)) {
          let dialogRef = this.dialog.open(CommonDialogComponent,
            { data: { title: 'dependency', name: 'dependency', content: "You don't Have an Access to View this file" }, panelClass: 'deletemod', width: '500px', disableClose: false });
          dialogRef.afterClosed().subscribe(res => {
            if (res) {
              if (this.profiledata.type == 'individual') this.router.navigate(['/individual/home/shareddocument'])
              else if (this.profiledata.type == 'organisation' || this.profiledata.type == 'employee') this.router.navigate(['/organization/home/shareddocument'])
...
/**
 ...
 async shared() {
    this.documentService.getSharedDoc(this.sharedid).subscribe(async res => {
      this.sharedRecord = res;
       if ((!this.sharedRecord.agreetoSign && !this.sharedRecord.view) || (!this.sharedRecord.agreetoReview && this.sharedRecord.view) &&this.sharedRecord && this.sharedRecord.fileid && !this.sharedRecord.folderid) {
         this.isloading=false
...
// To validate fields
  ...
  test() {
    ...
    else if (this.chatnotificationlogs.length == 0
      || (this.chatnotificationlogs.length &&
        this.chatnotificationlogs.some(x => x.documentid._id == this.sharedRecord.fileid._id))) {
...
// scroll for notifications
  ...
  */
  ...
  NotificationNavigate(data){
    ...
    if(data.documentid._id == this.sharedRecord.fileid._id) {
      this.newChat = true
      this.openChat = false
    }
    else{
